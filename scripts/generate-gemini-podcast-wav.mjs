...
if (
  /^\s*(TITLE|SUBTITLE|EPISODE|CHAPTER|OPEN|CLOSE|PAUSE|NARRATOR|VOICE ID|SOURCE|PRODUCTION|PRONUNCIATION|FORMAT|DELIVERY|FINAL QA)\b/im.test(
    text,
  )
) {
  ...
  for (const paragraph of paragraphs) {
    ...
    for (const rawSentence of sentences) {
      ...
      if (sentence.length > limit)
        throw new Error(
          "A narration sentence exceeds the safe Gemini chunk size.",
        );
...
const metadata = {
  ...
  status: "GENERATED DRAFT — HUMAN APPROVAL REQUIRED BEFORE PUBLICATION",
};
fs.writeFileSync(
  `${outputPath}.json`,
