...
for (let index = 0; index < chunks.length; index += 1) {
  ...
  if (!response.ok) {
    ...
    throw new Error(`Gemini TTS chunk ${index + 1}/${chunks.length} failed with ${response.status}: ${message}`);
  }

  const payload = await response.json();
  const part = payload?.candidates?.[0]?.content?.parts?.find((candidate) => candidate?.inlineData?.data);
  if (!part?.inlineData?.data) throw new Error(`Gemini TTS chunk ${index + 1} returned no audio data.`);
