...
const facebookCopyHtml = facebookPost ? `<section style="margin:30px 0;padding:22px;background:#101d30;border:1px solid #263b59;border-radius:16px"><h2>Facebook post — copy and paste</h2><p>Download the MP4 above, upload it to Facebook, then copy this approved post into the caption.</p><textarea id="facebook-post-copy" readonly style="box-sizing:border-box;width:100%;min-height:280px;padding:14px;border:1px solid #345274;border-radius:10px;background:#07111f;color:#eef4ff;font:15px/1.5 system-ui">${htmlEsc(facebookPost)}</textarea><p><button type="button" onclick="navigator.clipboard.writeText(document.getElementById('facebook-post-copy').value).then(() => { this.textContent='Copied to clipboard'; }).catch(() => { document.getElementById('facebook-post-copy').select(); })" style="padding:12px 18px;border:0;border-radius:999px;background:#66a7ff;color:#07111f;font-weight:750;cursor:pointer">Copy Facebook post</button></p><p style="color:#b8c9df">Human-review draft: check the wording before posting and reply personally to comments and questions.</p></section>` : "";
