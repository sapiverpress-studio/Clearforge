...
function page(title, description, pathname, body, structured) {
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(title)} | Sapiver Forge</title><meta name="description" content="${esc(description)}"><link rel="canonical" href="${esc(canonical(pathname))}"><meta property="og:type" content="article"><meta property="og:site_name" content="Sapiver Forge"><meta property="og:title" content="${esc(title)}"><meta property="og:description" content="${esc(description)}"><meta property="og:url" content="${esc(canonical(pathname))}"><meta property="og:image" content="${esc(canonical("/podcast/cover.png"))}"><meta name="twitter:card" content="summary_large_image"><link rel="stylesheet" href="/styles.css"><link rel="alternate" type="application/rss+xml" title="Sapiver Forge Features" href="/features.xml"><link rel="alternate" type="application/rss+xml" title="Sapiver Forge AI Briefing Podcast" href="/podcast/feed.xml"><script type="application/ld+json">${JSON.stringify(structured).replace(/</g, "\\u003c")}</script></head><body><header class="site-header"><a class="brand" href="/">Sapiver Forge</a><p>Human-led. AI-empowered.</p><nav aria-label="Site links"><a href="/guides/">Guides</a><a href="/topics/">Topics</a><a href="/reports/">Reports</a><a href="/podcast/">Podcast</a><a href="/newsletter/">Weekly digest</a></nav></header><main class="content">${body}</main><footer class="site-footer"><p>Turning human input into clear, usable systems.</p><p><a href="/guides/">Evergreen guides</a> · <a href="/topics/">Topics</a> · <a href="/podcast/">Podcast</a> · <a href="/newsletter/">Weekly digest</a></p></footer></body></html>`;
}
function newsletter() { return `<section class="newsletter-callout"><h2>Get the weekly Sapiver Forge digest</h2><p>One calm email covering what changed, why it matters and what is worth testing.</p><p><a class="button" href="/newsletter/">Join the weekly digest</a></p></section>`; }
...
for (const guide of guides) {
  ...
  const relatedHtml = related.length ? `<aside class="related"><h2>Related Sapiver Forge guides</h2><ul>${related.map((item) => `<li><a href="/guides/${item.slug}/">${esc(item.title)}</a></li>`).join("")}</ul></aside>` : "";
...
const indexBody = `<section class="hero"><p class="eyebrow">Sapiver Forge Evergreen Guides</p><h1>Durable answers beyond the daily AI headline.</h1><p>Plain-English guides explaining how AI adoption, products, systems and safeguards work in practice. Guides are updated when verified evidence changes.</p></section><section class="posts"><h2>Guides</h2>${cards}</section>${newsletter()}`;
write(path.join(GUIDES, "index.html"), page("Sapiver Forge Evergreen Guides", "Durable, source-backed Sapiver Forge explainers on AI adoption, everyday products, automation, models and accountability.", "/guides/", indexBody, { "@context": "https://schema.org", "@type": "CollectionPage", name: "Sapiver Forge Evergreen Guides", description: "Durable, source-backed explainers on practical AI.", url: canonical("/guides/") }));
...
console.log(`Published ${guides.length} Sapiver Forge evergreen guide${guides.length === 1 ? "" : "s"}.`);
