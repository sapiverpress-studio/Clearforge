...
function dailyVideoClientScript({ limit = 0 } = {}) {
  return `<script>(()=>{const host=document.querySelector('[data-daily-videos]');if(!host)return;const esc=s=>String(s||'');fetch('/api/daily-video/list').then(r=>{if(!r.ok)throw Error('Video list unavailable');return r.json()}).then(data=>{const items=${limit} ? data.items.slice(0,${limit}) : data.items;if(!items.length){host.textContent='No daily videos have been added yet.';return}host.replaceChildren(...items.map(item=>{const card=document.createElement('article');card.className='daily-video-card';const frame=document.createElement('iframe');frame.src='https://www.youtube-nocookie.com/embed/'+encodeURIComponent(item.videoId);frame.title=esc(item.title);frame.loading='lazy';frame.allow='accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share';frame.allowFullscreen=true;const h=document.createElement('h3');h.textContent=esc(item.title);const p=document.createElement('p');p.textContent=esc(item.date);const links=document.createElement('p');const youtube=document.createElement('a');youtube.href=item.youtubeUrl;youtube.textContent='Watch on YouTube';youtube.target='_blank';youtube.rel='noopener noreferrer';links.append(youtube);if(item.tiktokUrl){const sep=document.createTextNode(' · ');const tiktok=document.createElement('a');tiktok.href=item.tiktokUrl;tiktok.textContent='Open TikTok post';tiktok.target='_blank';tiktok.rel='noopener noreferrer';links.append(sep,tiktok)}card.append(frame,h,p,links);return card}))}).catch(()=>{host.textContent='The video archive is temporarily unavailable.'})})();</script>`;
}
function buildDailyVideos() {
  ...
  const uploadBody = `<section class="hero"><p class="eyebrow">Publisher tool</p><h1>Upload today’s finished TikTok video</h1><p>Save the captioned video from TikTok, select that MP4 here, and it will upload directly to the Sapiver Forge YouTube channel. The file is not stored in GitHub.</p></section><section class="posts publisher-upload"><form data-video-upload><p><label>Publisher password<br><input name="password" type="password" autocomplete="current-password" required></label></p><p><label>Finished TikTok MP4<br><input name="video" type="file" accept="video/mp4,.mp4" required></label></p><p><label>YouTube title<br><input name="title" maxlength="100" required></label></p><p><label>YouTube description<br><textarea name="description" rows="7" maxlength="5000"></textarea></label></p><p><label>TikTok post link (optional)<br><input name="tiktokUrl" type="url" inputmode="url" placeholder="https://www.tiktok.com/…"></label></p><p><label>Visibility<br><select name="privacyStatus"><option value="public">Public</option><option value="unlisted">Unlisted</option><option value="private">Private</option></select></label></p><p><button class="button" type="submit">Upload to YouTube and daily brief</button></p><progress data-upload-progress max="100" value="0" hidden></progress><p data-upload-status role="status" aria-live="polite"></p></form></section><script src="/daily-brief/video-upload.js" defer></script>`;
  write(path.join(publicDir, "daily-brief", "upload", "index.html"), pageTemplate("Upload daily video", "Private Sapiver Forge publisher tool for adding the finished TikTok video to YouTube and the daily brief.", uploadBody, { pathname: "/daily-brief/upload/" }));
