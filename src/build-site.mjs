...
function main() {
  ...
  const indexBody = `<section class="hero"><h1>Clear AI learning from noisy AI news.</h1><p>Clearforge explains what changed, who is adopting it, why it matters and what is worth testing.</p><div class="report-actions"><a class="button" href="/topics/">Explore topics</a><a class="button button-secondary" href="${podcastPage}">Listen to the podcast</a></div></section><section class="posts"><h2>Topic guides</h2><p>Follow durable coverage of AI adoption, everyday products, creator workflows, automation, models and accountability.</p><p><a class="button" href="/topics/">Browse all topics</a></p></section><section class="posts"><h2>Feature Analysis</h2>${features.length ? `<ul>${features.slice(0, 12).map((entry) => `<li><a href="${entry.url}">${escapeHtml(entry.title)}</a><span> — ${escapeHtml(entry.date)}</span></li>`).join("")}</ul>` : "<p>No approved features yet.</p>"}</section><section class="posts"><h2>Daily Briefs</h2>${briefs.length ? `<ul>${briefs.slice(0, 14).map((entry) => `<li><a href="${entry.url}">${escapeHtml(entry.title)}</a><span> — ${escapeHtml(entry.date)}</span></li>`).join("")}</ul>` : "<p>No approved public briefs yet.</p>"}</section><section class="posts"><h2>Clearforge Reports</h2><p>Go deeper with weekly AI learning briefs and standalone research papers.</p><p><a class="button" href="/reports/">Browse reports</a></p></section>${newsletterCallout()}`;
