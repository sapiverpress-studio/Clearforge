...
async function main() {
  ...
  const podcast = await generateStructured({
    ...
    prompt: `Create the weekly Sapiver Forge Intelligence Brief covering ${WEEK_START} to ${WEEK_END}.\n\nTarget a focused 3-6 minutes of spoken audio, roughly 400-850 words total. Choose only the four or five developments that mattered most across AI, technology, business, research and developer activity. Connect related developments and explain what changed over the week. End briefly with what is worth watching next. Do not pad the script or repeat facts simply to make it longer.\n\nDo not mention source URLs in spoken narration. Do not claim predictions are facts. Do not advertise products.\n\nVERIFIED DAILY EDITIONS:\n\n${buildInput(editions)}`,
    schema: PODCAST_SCHEMA
  });

  const transcript = makeTranscript(podcast);
  const wordCount = transcript.split(/\s+/).filter(Boolean).length;
  if (wordCount < 300) throw new Error(`Weekly podcast script is too short to be useful (${wordCount} words).`);
  ...
  const metadataCore = {
    episode: {
      ...
      estimated_duration_minutes: Math.max(3, Math.min(8, Number(podcast.estimated_duration_minutes || wordCount / 145))),
