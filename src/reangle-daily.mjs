...
async function main() {
  ...
  write(path.join(outDir, "structured_output.json"), JSON.stringify({ ...data, edition_id: DATE, base_date: BASE_DATE, edition_mode: "alternate_angle" }, null, 2));
  write(path.join(outDir, "alternate_angle_report.json"), JSON.stringify({ date: DATE, base_date: BASE_DATE, angle, reused_base_run: base.name, source_urls_reused: data.sources.map((s) => s.url), passed: true }, null, 2));
  ...
  console.log(`Clearforge alternate-angle edition ${DATE} created from ${BASE_DATE}/${base.name} using angle ${angle}.`);
}
