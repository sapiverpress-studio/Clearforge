...
async function main() {
  ...
  write(path.join(outDir, "editor_checklist.md"), `# Sapiver Forge Automatic QA Context — ${DATE}\n\n- This is an alternate-angle edition.\n- Source edition: ${BASE_DATE}.\n- Editorial theme: ${theme.day} — ${theme.title}.\n- Same source material may be reused deliberately.\n- Validate the article and feature against the same confirmed source facts.\n`);
  console.log(`Sapiver Forge alternate-angle edition ${DATE} created from ${BASE_DATE}/${base.name} using angle ${angle} and ${theme.day} theme.`);
}
