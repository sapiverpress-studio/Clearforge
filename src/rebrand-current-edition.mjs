...
const roots = [path.join(ROOT, "drafts", edition), path.join(ROOT, "media", edition)];
const textExtensions = new Set([".txt", ".md", ".json", ".html", ".htm", ".xml", ".js", ".mjs", ".css"]);
...
for (const file of files) {
  ...
  replacements += (original.match(/\bclear\s*forge\b/gi) || []).length;
...
for (const file of files) {
  ...
  const withoutExternalTokens = content.replace(
    /https?:\/\/[^\s"'<>)]*|\b[^\s@]+@[^\s@]+\.[^\s@]+\b/gi,
    "",
  );
  if (/\bclear\s*forge\b/i.test(withoutExternalTokens)) remaining.push(path.relative(ROOT, file));
}
