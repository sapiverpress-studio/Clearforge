...
if (originalSources.length < 1 || originalSources.length > 5) throw new Error("Source resolution requires between one and five source candidates.");
...
function selectGroundedEvidence(resolved, source, evidence, evidenceByUrl) {
  ...
  const ranked = evidence
    .map((item) => ({ item, score: Math.max(titleSimilarity(resolved.title, item.title), titleSimilarity(source.title, item.title)) }))
...
for (const item of evidence) { try { evidenceByUrl.set(normaliseUrl(item.url), item); } catch {} }
...
for (let index = 0; index < originalSources.length; index += 1) {
  ...
  replacements.push({ original_index: index, old_url: source.url || "", new_url: evidenceItem.url, proposed_canonical_url: resolved.url, old_title: source.title || "", new_title: canonicalTitle, evidence_url: evidenceItem.url, evidence_title: evidenceItem.title, evidence_grounded: true, match_type: grounded.match_type });
}
...
if (survivors.length < 1) throw new Error(`None of ${originalSources.length} source candidates survived resolution.`);
...
for (const item of dropped) console.warn(`DROPPED SOURCE ${item.original_index + 1}: ${item.title} — ${item.reason}`);
