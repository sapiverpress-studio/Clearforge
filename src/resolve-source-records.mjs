...
function collectSearchEvidence(value, out = []) {
  ...
  if (typeof value.url === "string" && /^https:\/\//i.test(value.url)) {
    out.push({ url: value.url, title: String(value.title || value.name || "") });
  }
...
const response = await client.responses.create({
  ...
  input: [
    { role: "system", content: "Resolve citations only. Search for an exact real page supporting each confirmed fact. Select only a URL returned by the search tool. Never construct or edit a slug. Prefer primary sources, otherwise exact reputable reporting. If the story cannot be sourced, replace it with a current verifiable story in the same lane and category." },
