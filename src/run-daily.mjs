...
const today = process.env.SAPIVER_FORGE_DATE || new Intl.DateTimeFormat("sv-SE", {
...
function editorialTheme(dateValue) {
  ...
  const themes = {
    ...
    Saturday: {
      slug: "sapiver-forge-forecast",
      title: "Sapiver Forge forecast",
    ...
    Sunday: {
      ...
      focus: "Recap the strongest developments of the week, check any recent Sapiver Forge forecasts where evidence exists, and prepare the reader for the following week.",
...
async function main() {
  ...
  if (campaignActive) writeCampaignRecord(outDir, { edition: today, campaign_id: campaign.id, status: "broad_pack_generated_pending_social_routing", content_theme: "broad_ai_podcast", source_url: enrichedData.sources[0]?.url || "", product_page_url: campaign.product_url, api_activity: { exa_queries: acquisition.query_count, gemini_text_calls: generationAttempts }, source_cost_usd: acquisition.provider_cost_usd, content_rejected_before_publication: 0 });
