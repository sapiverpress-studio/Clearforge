...
function extractTitle(html) {
  return extractMeta(html, "og:title") || extractMeta(html, "twitter:title") || decodeEntities(html.match(/<title[^>]*>([\s\S]*?)<\/title>/i)?.[1] || "").replace(/\s+/g, " ").trim();
}
...
function materialNumbers(value) {
  return [...new Set(String(value || "").match(/\b\d+(?:\.\d+)?(?:%|B|M|K|bn|million|billion)?\b/gi) || [])].filter((item) => !/^20\d{2}$/.test(item));
}
function importantTerms(source) {
  const found = `${source.title || ""} ${source.confirmed_fact || ""}`.match(/\b[A-Z][A-Za-z0-9.-]{2,}(?:\s+[A-Z][A-Za-z0-9.-]{2,}){0,3}\b/g) || [];
  return [...new Set(found.map(normalize).filter((value) => value.length > 3))].slice(0, 8);
}
...
for (let index = 0; index < sources.length; index += 1) {
  ...
  let finalUrl = "", canonicalUrl = "", pageTitle = "", body = "", status = 0;
  try {
  ...
  if (pageTitle && titleScore < 0.45) failures.push(`Recorded title does not match resolved page title (similarity ${titleScore.toFixed(2)}).`);
...
const report = {
  ...
  policy: "Fail closed: every source URL must resolve, match its title and contain the material evidence used by the draft.",
  results
};
